...
$("#OCid_cmb_suc,#OCid_cmb_alm,#OCtipo,#OCid_cmb_prov,#OCnro,#OCfecha,#OCreferencia,#OCentrega,#OCanticipo,#OCid_cmb_pro,#OCcantidad").attr("disabled", true);
$("#OCbtn_guardar,#OCbtn_imprimir,#OCbtn_limpiar,#OCbtn_anular,#OCbtn_finalizar").attr("disabled", true);


$("#OCbtn_nuevo").click(function () {    $("#OCid_cmb_suc,#OCid_cmb_alm,#OCtipo,#OCid_cmb_prov,#OCfecha,#OCreferencia,#OCentrega,#OCanticipo,#OCid_cmb_pro,#OCcantidad").attr("disabled", false);
    NroOrdCom()
    $("#OCbtn_guardar,#OCbtn_limpiar").attr("disabled", false);
    $("#OCbtn_buscar").attr("disabled", true)
});
...
$("#OCbtn_imprimir").click(function () {
    ...
    if ($("#id_OC").val() == '') { swal("Seleccione una orden de compra", "", "warning")return false;  }
...
$("#OCbtn_cancelar").click(function () {
    ...
    $("#OCbtn_guardar,#OCbtn_imprimir,#OCbtn_limpiar,#OCbtn_finalizar,#OCbtn_anular,#OCentrega,#OCanticipo,#OCid_cmb_prov").attr("disabled", true);
...
function OClistar() {
    ...
    for (var i = 0; i < orden_compra.length; i++) {
        ...
        $("#IdCuerpoOCD").append("<tr><td class='text-left' width='3%'><a class=' text-left' onclick='OCeliminar(" + i + ")'>\n\
<icon class='fa fa-trash'> </icon></a></td><td width='3%'>" + parseInt(i + 1) + "</td>\n\
<td style='text-align:left;' width='50%'>" + orden_compra[i].nombre_producto + "</td>\n\
<td style='text-align:left;' width='4%'> " + orden_compra[i].cantidad + "</td>\n\
<td width='10%'> " + orden_compra[i].unidad + "</td><td width='10%'> " + orden_compra[i].despachado + "</td><td width='10%'> " + orden_compra[i].pendiente + "</td>\n\
<td width='8%'> <input style='text-align:right; width:70px' class='appendform-control' id='fila_"+i+"' onchange='ChangePrecio("+i+")' value='" + orden_compra[i].precio + "' ></td>");
    }
}
...
function almacenxsucursal() {
    ...
    $.post("controlador/Clogistica.php?op=LISTAR_ALMxSUC", {sucursal: $("#OCid_cmb_suc").val()}, function (data) {
        //console.log(data)
...
function OCguardar() {
    ...
    if ($("#OCnro").val() == "") {
        swal("Campo requerido", "Inserte número", "warning");
        $("#OCnro").focus();
        return false;
    }
	...
	if ($("#OCentrega").val() == "") {
        swal("Campo requerido", "Inserte plazo de entrega", "warning");
        $("#OCentrega").focus();
        return false;
    }
    if ($("#OCanticipo").val() == "") {
        swal("Campo requerido", "Inserte anticipo", "warning");
        $("#OCanticipo").focus();
        return false;
    }
...
function OCcancelar() {
   ...
   /* $("#OCcategoria").val("").change();*/
    ...
    setTimeout(function () {        $("#OCid_cmb_alm,#OCid_cmb_suc,#OCreferencia,#OCfecha,#OCtipo,#OCid_cmb_pro,#OCcantidad,#OCid_cmb_prov,#OCentrega,#OCanticipo").attr("disabled", true);
    }, 100);
    OClistar();
}
...
function almacenxsucursal2() {
    ...
    $.post("controlador/Clogistica.php?op=LISTAR_ALM_GRALxSUC", {sucursal: $("#OCbuscar-id_cmb_suc").val()}, function (data) {
